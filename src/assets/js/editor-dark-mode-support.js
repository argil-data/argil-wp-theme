/* global argilfolioxIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'argilfoliox-supports-dark-theme' ) ) {
	argilfolioxDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function argilfolioxDarkModeEditorInit( attempt ) {
	...
	if ( argilfolioxIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						argilfolioxDarkModeEditorInit( attempt + 1 );
					},
